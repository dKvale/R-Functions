...
ozone_Booted <- boot_NADA(data=ozone, results="Ozone", censored="Non_detect", groups = "Month", repeats=100)
head(ozone_Booted)
...
boot_NADA <- function(data = dataTable, results = "result", censored = "censored", groups = "groupID", repeats = 2000, percentile = 0.95){
  ...
  for (group in unique(data$groupID)){
    ...
    bootedMeans  <- lapply(1:repeats, FUN= function(x) mean(cenfit(subGroup_table$result[sample(1:n, replace=T)], subGroup_table$censored))[1])
